...
const minesEncrypt = require("../model/games/mines/mine_encrypt")
const { format } = require('date-fns');
const minesgameInit = require('../model/games/mines/minesgameInit');
...
const handleMinesEncryption = (async(req, res)=>{
  try{
const user_id  = req.id
  let mimeEncription =   await minesEncrypt.findOne({user_id})
  if(!mimeEncription){
     mimeEncription = await InitializeMinesGame(user_id)
  }
    res.status(200).json(mimeEncription)
  }
    catch(err){
